...
bool NFCNet::Dismantle(NetObject* pObject)
{
    ...
    if (len > NFIMsgHead::NF_HEAD_LENGTH)
    {
        NFCMsgHead xHead;
        ...
        if (nMsgBodyLength > 0 && xHead.GetMsgID() > 0)
        {
            ...
            if (mRecvCB)
            {
                mRecvCB(pObject->GetRealFD(), xHead.GetMsgID(), pObject->GetBuff() + NFIMsgHead::NF_HEAD_LENGTH, nMsgBodyLength);
            }

			pObject->RemoveBuff(0, nMsgBodyLength + NFIMsgHead::NF_HEAD_LENGTH);
...
bool NFCNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const int nSockIndex /*= 0*/)
{
    ...
    if (nAllLen == nLen + NFIMsgHead::NF_HEAD_LENGTH)
    {
        //打包成功
...
int NFCNet::EnCode(const uint16_t unMsgID, const char* strData, const uint32_t unDataLen, std::string& strOutData)
{
    ...
    char szHead[NFIMsgHead::NF_HEAD_LENGTH] = { 0 };
    xHead.EnCode(szHead);

    strOutData.clear();
    strOutData.append(szHead, NFIMsgHead::NF_HEAD_LENGTH);
    strOutData.append(strData, unDataLen);

    return xHead.GetBodyLength() + NFIMsgHead::NF_HEAD_LENGTH;
}

int NFCNet::DeCode(const char* strData, const uint32_t unAllLen, NFCMsgHead& xHead)
{
    //解密--unLen为buff总长度,解包时能用多少是多少
    if (unAllLen < NFIMsgHead::NF_HEAD_LENGTH)
    {
        //长度不够
        return -1;
    }

    if (NFIMsgHead::NF_HEAD_LENGTH != xHead.DeCode(strData))
    {
        //取包头失败
        return -2;
    }

    if (xHead.GetBodyLength() > (unAllLen - NFIMsgHead::NF_HEAD_LENGTH))
    {
        //总长度不够
        return -3;
    }
