...
MemoryDataStream::MemoryDataStream(void* pMem, size_t inSize, bool freeOnClose, bool readOnly)
	: DataStream(static_cast<USHORT>(readOnly ? READ : (READ | WRITE)))
{
...
MemoryDataStream::MemoryDataStream(const string& name, void* pMem, size_t inSize, 
	...
	: DataStream(name, static_cast<USHORT>(readOnly ? READ : (READ | WRITE)))
{
	mData = mPos = static_cast<UCHAR*>(pMem);
	mSize = inSize;
	mEnd = mData + mSize;
	...
	const string& delim)
{
	if (delim.empty())
	{
		assert(0 && "No delimiter provided FileStreamDataStream::readLine");
	}
	if (delim.size() > 1)
	{
		assert(0 && "WARNING: FileStreamDataStream::readLine - using only first delimeter");
	}
	// Deal with both Unix & Windows LFs
	bool trimCR = false;
	if (delim.at(0) == '\n') 
	...
	else if (mInStream->fail())
	{
		...
		{
			assert(0 && "Streaming error occurred FileStreamDataStream::readLine");
		}
	}
	else 
	{
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
FileHandleDataStream::FileHandleDataStream(FILE* handle, USHORT accessMode)
...
//-----------------------------------------------------------------------
FileHandleDataStream::FileHandleDataStream(const string& name, FILE* handle, USHORT accessMode)
